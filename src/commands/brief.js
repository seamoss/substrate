...
import { generateOverflowSummary, generateBudgetFooter } from '../lib/summarize.js';
...
/**
 * Generate CLAUDE.md-compatible output format.
 *
 * Creates output designed for direct injection into CLAUDE.md files,
 * bridging the instructions and MCP strategies.
 *
 * @param {Brief} brief - The structured brief object
 * @param {import('../db/local.js').Workspace} workspace - Workspace info
 * @returns {string} CLAUDE.md-compatible output
 * @private
 */
function generateClaudeMdFormat(brief, workspace) {
  const lines = [];

  lines.push(`# Substrate Context: ${workspace.name}`);
  lines.push('');
  lines.push('> Auto-generated by `substrate brief --format claudemd`. Do not edit manually.');
  lines.push('');

  if (brief.constraints.length > 0) {
    lines.push('## Constraints');
    lines.push('');
    lines.push('IMPORTANT: These are hard requirements that must be followed:');
    lines.push('');
    brief.constraints.forEach(c => {
      lines.push(`- ${c.content}`);
    });
  ...
  if (brief.decisions.length > 0) {
    lines.push('## Architectural Decisions');
    lines.push('');
    lines.push('Follow these architectural choices that have already been made:');
    lines.push('');
    brief.decisions.forEach(d => {
      lines.push(`- ${d.content}`);
    });
  ...
  lines.push('---');
  lines.push(`*${brief.count} context items from Substrate workspace "${workspace.name}"*`);
...
export const briefCommand = new Command('brief')
  ...
  .action(async (path, options) => {
    ...
    // Parse budget if specified
    const budget = options.budget ? parseBudget(options.budget) : null;
    if (options.budget && budget === null) {
      console.error(
        `Invalid budget '${options.budget}'. Use a number or preset: ${Object.keys(BUDGET_PRESETS).join(', ')}`
      );
      process.exit(1);
    }

    // Apply priority scoring and budget fitting
    let itemsForBrief = filtered;
    let overflowItems = [];
    let budgetInfo = null;

    if (budget) {
      // Build scoring context
      const filterTags = options.tag ? options.tag.split(',').map(t => t.trim()) : [];
      ...
      const ranked = rankItems(filtered, scoringContext);

      // Fit to budget
      const result = fitToBudget(ranked, budget, { linkMap });
      itemsForBrief = result.included;
    ...
    const brief = {
      ...
      path: targetPath,
      constraints: itemsForBrief
        .filter(i => i.type === 'constraint')
        .map(i => slimContext(i, linkMap)),
      decisions: itemsForBrief.filter(i => i.type === 'decision').map(i => slimContext(i, linkMap)),
      notes: itemsForBrief.filter(i => i.type === 'note').map(i => slimContext(i, linkMap)),
      tasks: itemsForBrief.filter(i => i.type === 'task').map(i => slimContext(i, linkMap)),
      entities: itemsForBrief.filter(i => i.type === 'entity').map(i => slimContext(i, linkMap)),
      count: itemsForBrief.length
    };

    // Generate prompt text
    let prompt = generatePrompt(brief, linkMap, itemsForBrief);

    // Append overflow summary and budget footer if budget was specified
    if (budget) {
      const overflowSummary = generateOverflowSummary(overflowItems);
      if (overflowSummary) {
        prompt += '\n\n' + overflowSummary;
      }
      prompt += generateBudgetFooter(
        budgetInfo.tokensUsed,
        budgetInfo.budget,
        budgetInfo.includedCount,
    ...
    if (options.format === 'claudemd') {
      console.log(generateClaudeMdFormat(brief, workspace));
      return;
    }
    ...
    if (brief.constraints.length > 0) {
      ...
      itemsForBrief.filter(i => i.type === 'constraint').forEach(printWithLinks);
      console.log();
    }

    if (brief.decisions.length > 0) {
      console.log(chalk.yellow.bold('Decisions:'));
      itemsForBrief.filter(i => i.type === 'decision').forEach(printWithLinks);
      console.log();
    }

    if (brief.notes.length > 0) {
      console.log(chalk.blue.bold('Notes:'));
      itemsForBrief.filter(i => i.type === 'note').forEach(printWithLinks);
      console.log();
    }

    if (brief.tasks.length > 0) {
      console.log(chalk.magenta.bold('Tasks:'));
      itemsForBrief.filter(i => i.type === 'task').forEach(printWithLinks);
      console.log();
    }

    if (brief.entities.length > 0) {
      console.log(chalk.cyan.bold('Entities:'));
      itemsForBrief.filter(i => i.type === 'entity').forEach(printWithLinks);
      console.log();
    }
    ...
    if (brief.count === 0) {
      ...
      if (budgetInfo) {
        dim(
          `${budgetInfo.tokensUsed.toLocaleString()}/${budgetInfo.budget.toLocaleString()} tokens | ${budgetInfo.includedCount}/${budgetInfo.totalCount} items`
        );
      }
    }
  });
